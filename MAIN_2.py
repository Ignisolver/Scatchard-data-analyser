...
while True:
    ...
    while True:
        ...
        str_funkcje = ['1. Pokaż podsumowanie grupy','2.Pokaż wykres grupy i jej dane',
                       "3.Pokaż zestawienie jednego szczura z innymi","4. Pokaż wykres pojedyńczego szczura i jego dane",
        ...
        if nr_fun == 2:
            if input("Czy chcesz podać zakres? (enter - nie / cos innego tak)>>"):
                print("gdy niechcesz uwzględnić jakiejś osi naciśnij dwa razy enter")
                x_min = ii("podaj x_min >>")
                x_max = ii("podaj x_max >>")
                y_min = ii("podaj y_min >>")
                y_max = ii("podaj y_max >>")
                zakres = [[x_min,x_max],[y_min,y_max]]
            else:
                zakres = False
            sel_group.wykres_grupy(zakres)
            a = input("nacisnij enter aby kontynuować")
            continue
        ...
        if nr_fun == 4:
            ...
            for nr_pkt_w_szczurze, [b, f, ok] in enumerate(zip(sel_group.szczury[nr_szcz].By, sel_group.szczury[nr_szcz].Fy,
            ...
            sel_group.szczury[nr_szcz].wykres_szczura(zakres)
            a = input("nacisnij enter aby kontynuować")
            continue
        ...
        if nr_fun == 8:
            if input("czy chcesz wybrac grupy do dolaczenia? (enter - tak, cos innego - nie>>"):
                ...
                if -1 in grups_to_add:
                    ...
                    continue
                grups = [grupy[gn(i)] for i in grups_to_add]
